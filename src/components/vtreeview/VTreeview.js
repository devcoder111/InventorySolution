...
export default {
  ...
  computed: {
    allDescendantLeafs () {
      ...
      const searchTree = items => {
        const children = this.getChildrenArray(items)
  ...
  methods: {
          ...
          change: selection => {
            ...
            if (this.hasChildren) {
              if (this.allDescendantLeafsSelected) {
                // deselect all
                const ix = this.value.indexOf(this.items[this.keyField])
                if (ix > -1) {
      ...
      return this.$createElement(VTreeview, {
        on: {
          ...
          uncheck: childId => {
            ...
            // uncheck the child
            const cix = this.value.indexOf(childId)
            if (cix > -1) {
    ...
    async toggleExpandTree () {
      const ix = this.expandedItems.indexOf(this.items[this.keyField])
      if (ix > -1) {
        ...
        if (this.hasDynamicChildren) {
          ...
          const loaded = await this.items.children(this.dynamicChildrenContext)
          if (!loaded) {
    ...
    uncheckAllChildren () {
      for (var i = 0; i < this.allDescendantLeafs.length; i++) {
        const ix = this.value.indexOf(this.allDescendantLeafs[i][this.keyField])
        if (ix > -1) {
          this.spliceOne(this.value, ix)
        }
      }
      for (i = 0; i < this.children.length; i++) {
        const ix = this.value.indexOf(this.children[i][this.keyField])
        if (ix > -1) {
          this.spliceOne(this.value, ix)
        }
      }

    ...
    <div>
      <div class="home-content">
        <v-container style="max-width: 100%; padding: 0">
          ...
          >
            <v-col class="py-0" cols="12" sm="4">
              <span class="text-h6 sub-title">Edit Folder</span>
            </v-col>
          </v-row>
          <v-form
              ...
              <v-col style="padding-right: 45px">
                ...
                <div class="text-left pt-10">
                  <v-btn
                    ...
                    medium
                    :disabled="!valid"
                    class="edit-folder-btn save-btn"
                  >
                    Save
                  </v-btn>
                  <v-btn
                    ...
                    <div class="input-mb input-box">
                      ...
                      >
                        <vue-upload-multiple-image
                          ...
                          v-bind:dataImages="folderToSubmit.images"
                          :multiple-flag="false"
...
export default {
  ...
  async created() {
    ...
    await this.getCategory(this.categoryId).then((res) => {
      this.category = res
      this.tmpCategoryId = res.id
      this.folderToSubmit.name = res.name
      this.folderToSubmit.notes = res.description

      const formData = new FormData()
      formData.append('file', res.image)
      if (res.image == null) {
        this.folderToSubmit.images = []
      } else {
        this.folderToSubmit.images.splice(0, 1, {
          path: res.image,
          highlight: 1
        })
      }
      this.curUserInventoryId = res.inventoryId
    })
    this.$store.commit(
  ...
  methods: {
    ...
    beforeRemove(index, done, fileList) {
      this.folderToSubmit.images[0] = null
      this.imageFormData = ''
      this.changedFlag = true
    },
    ...
    async validate() {
      ...
      let image
      if (
        Array.isArray(this.folderToSubmit.images) &&
        this.folderToSubmit.images.length
      ) {
        image = this.folderToSubmit.images[0].path
      }
      ...
      const payload = {
        id: this.tmpCategoryId,
        inventoryId: this.curUserInventoryId,
        name: this.folderToSubmit.name,
        parentId: !this.folderToSubmit.folder
          ? null
          : this.folderToSubmit.folder.id,
